...
# 3. 修改 consumable_withdraw 表 (添加 bill_id)
try:
    cursor.execute('''
        ALTER TABLE consumable_withdraw
        ADD COLUMN bill_id INT NULL,
        ADD CONSTRAINT fk_consumable_withdraw_bill FOREIGN KEY (bill_id) REFERENCES bill(id) ON DELETE SET NULL
    ''')
    print('✓ consumable_withdraw 添加 bill_id 成功')
except Exception as e:
    if 'Duplicate column' in str(e):
        print('✓ consumable_withdraw 已存在 bill_id')
    else:
        print(f'✗ consumable_withdraw 添加 bill_id 失败: {e}')

# 4. 修改 staff_charge 表 (添加 bill_id 和 amount)
try:
    cursor.execute('''
