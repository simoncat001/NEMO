...
```
GET    /api/v1/config/configurations                    获取配置列表
POST   /api/v1/config/configurations                    创建配置 ⚠️ 管理员
GET    /api/v1/config/configurations/{id}               获取配置详情
PUT    /api/v1/config/configurations/{id}               更新配置 ⚠️ 管理员
DELETE /api/v1/config/configurations/{id}               删除配置 ⚠️ 管理员
POST   /api/v1/config/configurations/{id}/change-setting 修改配置设置 ⚠️ 维护人员
GET    /api/v1/config/configurations/tool/{id}/list     获取工具配置列表
GET    /api/v1/config/configurations/stats              获取统计信息 ⚠️ 管理员
```

#### ConfigurationOption 端点 (5个)

```
GET    /api/v1/config/configuration-options             获取配置选项列表
POST   /api/v1/config/configuration-options             创建配置选项
GET    /api/v1/config/configuration-options/{id}        获取配置选项详情
PUT    /api/v1/config/configuration-options/{id}        更新配置选项
DELETE /api/v1/config/configuration-options/{id}        删除配置选项
```

#### ConfigurationHistory 端点 (4个)

```
GET    /api/v1/config/configuration-history             获取配置历史列表
GET    /api/v1/config/configuration-history/{id}        获取历史详情
POST   /api/v1/config/configuration-history             创建历史记录 ⚠️ 管理员
```
...
```python
# 创建激光功率配置
POST /api/v1/config/configurations
{
  "name": "激光功率",
  "tool_id": 1,
...
# 修改配置（选择 150W）
POST /api/v1/config/configurations/1/change-setting
{
  "slot": 0,
  "choice": 2
}
...
# 查看历史
GET /api/v1/config/configuration-history?configuration_id=1
```
...
# 修改第2个槽位（索引1）为 532nm
POST /api/v1/config/configurations/2/change-setting
{
  "slot": 1,
  "choice": 1
}
...
```python
# 创建预约时保存配置选项
POST /api/v1/config/configuration-options
{
  "name": "激光功率",
  "configuration_id": 1,
  "reservation_id": 100,
...
# 查询预约的配置选项
GET /api/v1/config/configuration-options?reservation_id=100
```

### 场景 4: 配置统计
```python
# 查看工具配置统计
GET /api/v1/config/configurations/stats?tool_id=1&days=30
...
4. `backend/app/api/v1/endpoints/configurations.py` - 17个 API 端点
...
5. `backend/app/api/v1/api.py` - 注册新路由
...
```bash
# 创建配置
curl -X POST http://localhost:8000/api/v1/config/configurations \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"name": "激光功率", "tool_id": 1, ...}'

# 修改配置设置
curl -X POST http://localhost:8000/api/v1/config/configurations/1/change-setting \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"slot": 0, "choice": 2}'

# 获取统计
curl http://localhost:8000/api/v1/config/configurations/stats \
  -H "Authorization: Bearer $TOKEN"
```
