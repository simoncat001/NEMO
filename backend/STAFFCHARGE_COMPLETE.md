...
```
GET    /api/v1/staff/staff-charges                    获取收费记录列表
POST   /api/v1/staff/staff-charges                    开始服务（创建记录）
GET    /api/v1/staff/staff-charges/{id}               获取记录详情
POST   /api/v1/staff/staff-charges/{id}/end           结束服务
PUT    /api/v1/staff/staff-charges/{id}               更新记录
DELETE /api/v1/staff/staff-charges/{id}               删除记录
GET    /api/v1/staff/staff-charges/staff/{id}/active  员工当前服务
GET    /api/v1/staff/staff-charges/customer/{id}/active 客户当前接受的服务
POST   /api/v1/staff/staff-charges/{id}/validate      验证记录
POST   /api/v1/staff/staff-charges/{id}/waive         豁免记录
GET    /api/v1/staff/staff-charges/stats              统计数据
```
...
```python
# 1. 员工开始协助客户
POST /api/v1/staff/staff-charges
{
  "staff_member_id": 5,
  "customer_id": 10,
  "project_id": 3,
...
# 2. 服务结束
POST /api/v1/staff/staff-charges/123/end
{
  "note": "设备设置完成"
}

# 3. 管理员验证
POST /api/v1/staff/staff-charges/123/validate
```

### 场景 2: 查询员工工作量
```python
# 查询某员工的所有服务记录
GET /api/v1/staff/staff-charges?staff_member_id=5&start_date=2025-12-01

# 查看员工当前正在进行的服务
GET /api/v1/staff/staff-charges/staff/5/active
```

### 场景 3: 项目收费统计
```python
# 按项目统计收费
GET /api/v1/staff/staff-charges/stats?project_id=3
...
4. `backend/app/api/v1/endpoints/staff_charges.py` - API 端点

**修改的文件**:
1. `backend/app/models/user.py` - 添加关系
2. `backend/app/models/__init__.py` - 导出新模型
3. `backend/app/api/v1/api.py` - 注册新路由
...
```bash
# 开始服务
curl -X POST http://localhost:8000/api/v1/staff/staff-charges \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"staff_member_id": 1, "customer_id": 2, "project_id": 1}'

# 结束服务
curl -X POST http://localhost:8000/api/v1/staff/staff-charges/1/end \
  -H "Authorization: Bearer $TOKEN"

# 获取统计
curl http://localhost:8000/api/v1/staff/staff-charges/stats \
  -H "Authorization: Bearer $TOKEN"
```
