...
def _build_model_serializer_map():
    model_serializer_map = {}
    for serializer_type in list(globals().values()):
        if not isinstance(serializer_type, type):
            continue
        if not issubclass(serializer_type, serializers.ModelSerializer):
            continue
        meta = getattr(serializer_type, "Meta", None)
        model = getattr(meta, "model", None)
        if model is not None:
            model_serializer_map[model] = serializer_type
    return model_serializer_map


MODEL_SERIALIZER_MAP = _build_model_serializer_map()


def _get_serializer_for_model(model_cls):
    serializer_type = MODEL_SERIALIZER_MAP.get(model_cls)
    if serializer_type:
        return serializer_type
    for registered_model, serializer in MODEL_SERIALIZER_MAP.items():
        if issubclass(model_cls, registered_model):
            return serializer
    return None


def serialize_model_instance(instance):
    serializer_type = _get_serializer_for_model(instance.__class__)
    if serializer_type:
        return serializer_type(instance).data
    return model_to_dict(instance)


def serialize_queryset(queryset):
    model_cls = queryset.model
    serializer_type = _get_serializer_for_model(model_cls)
    if serializer_type:
        return serializer_type(queryset, many=True).data
    return [model_to_dict(instance) for instance in queryset]


def serialize_context(value):
    if isinstance(value, QuerySet):
        return serialize_queryset(value)
    if isinstance(value, Model):
        return serialize_model_instance(value)
    if isinstance(value, dict):
        return {key: serialize_context(item) for key, item in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [serialize_context(item) for item in value]
